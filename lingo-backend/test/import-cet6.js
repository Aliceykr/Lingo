...
function fetchWordlist() {
  return new Promise((resolve, reject) => {
    https.get('https://raw.githubusercontent.com/mahavivo/english-wordlists/master/CET6_edited.txt', (res) => {
      let data = '';
...
async function main() {
  ...
  const [wbs] = await connection.query("SELECT id FROM wordbooks WHERE name = 'CET-6' LIMIT 1");
  if (wbs.length === 0) {
    console.error('找不到 CET-6 单词本');
    process.exit(1);
  }
